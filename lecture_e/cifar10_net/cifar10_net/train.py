...
from cifar10_net.data import CIFAR10DataModule, get_default_data_root
from cifar10_net.model import PLCIFARModule
from cifar10_net.utils import get_wandb_key, args_to_flat_dict
...
def main(args):
    ...
    args.trainer.logger = WandbLogger(
        project="ms-in-dnns-cifar10-lightning", name=args.run_name, save_dir=wandb_save_dir
    )
    args.trainer.logger.experiment.config.update(args_to_flat_dict(args))

    dm = CIFAR10DataModule(**vars(args.data))
    model = PLCIFARModule(**vars(args.model))
...
if __name__ == "__main__":
    ...
    parser.set_defaults({"trainer.max_epochs": 60, "trainer.num_sanity_val_steps": 2})

    parser.add_lightning_class_args(PLCIFARModule, "model")

    parser.add_lightning_class_args(CIFAR10DataModule, "data")
    parser.set_defaults({"data.data_root": get_default_data_root()})
