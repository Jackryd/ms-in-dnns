# See report here: https://wandb.ai/jackryd-chalmers-university-of-technology/ms-in-dnns-cifar10-adv-attacks/reports/Assignment-D--VmlldzoxNTkyODI4OA
"""
...
def get_attack_loader(val_dataset, source_class: str, n_samples: int) -> DataLoader:
    ...
    for i, (_, label) in enumerate(val_dataset):
        if int(label) == source_idx:
            selected.append(i)
            if len(selected) == n_samples:
                break
    subset = torch.utils.data.Subset(val_dataset, selected)
...
def compute_adversarial(
    ...
    optimizer = optim.Adam([image_copy], lr=lr, maximize=True)
    prob = None

    for i in range(max_iter):
        output = model(image_copy)
        prob_t = torch.softmax(output, dim=1)[0, target_class]
...
def main(args):
    ...
    for image, label in attack_loader: 
        ...
        for target_class in range(10):
            adv, prob = compute_adversarial(model, image, target_class, args.lr, args.max_iter, args.prob_threshold)
            img_diff = (image-adv).abs() * 10
            result_tbl.add_data(wandb.Image(to_vis(image[0])), args.source_class, CLASS_NAMES[target_class], wandb.Image(to_vis(adv[0])), wandb.Image(to_vis(img_diff[0])), prob)
